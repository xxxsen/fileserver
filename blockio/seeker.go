...
func NewSeeker(ctx context.Context, b2f BlockIdToFileKeyConvertFunc, fsize int64) ISeeker {
...
func (s *defaultSeeker) openStream(at int64) (io.ReadCloser, error) {
	if at == s.fsize {
...
func (s *defaultSeeker) Seek(offset int64, whence int) (ret int64, err error) {
	if !s.isOpen {
...
func (s *defaultSeeker) calcOffset(offset int64, whence int) int64 {
	cur := int64(s.cur)
	switch whence {
	case io.SeekStart:
		cur = offset
...
func (s *defaultSeeker) Close() error {
	if !s.isOpen {
		return nil
	}
	if s.rc == nil {
...
func (s *defaultSeeker) Read(b []byte) (int, error) {
	if !s.isOpen {
		return 0, fmt.Errorf("file not in open state")
	}
	if s.rc == nil {
